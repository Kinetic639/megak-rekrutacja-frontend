...
export const FiltersPanel: React.FC<FiltersPanelProps> = ({
  ...
  const initUIFilterOptionsActiveStatus = () => {
    elementsRef.current.forEach((elem) => {
      ...
      if (
        ...
        currFilterValue.length &&
        currFilterValue.find((item) => item === `${elem.dataset.value}`)
      ) {
  ...
  const ratingsContents = useCallback(
    (filterName: string) =>
      new Array(5).fill(filterName).map((item, index, arr) => {
        return (
          <button
            className="filtersPanel__option-btn"
            key={`${item}${index}`}
            ref={(e: any) => elementsRef.current.push(e)}
            data-filter={item}
            data-value={`${arr.length - index}`}
            onClick={setFilter}
          >
  ...
  const workPlaceContents = useCallback(
    (filterName: string) =>
      [
  ...
  const workTypeContents = useCallback(
    (filterName: string) =>
      [
  ...
  return (
    <>
      ...
      <article className="filtersPanel">
        ...
        <Filter
          subTitle="Ocena przejścia kursu"
          listComponents={ratingsContents('courseCompletion')}
        />
        <Filter
          subTitle="Ocena aktywności i zaangażowania w kursie"
          listComponents={ratingsContents('courseEngagement')}
        />
        <Filter
          subTitle="Ocena kodu w projekcie własnym"
          listComponents={ratingsContents('projectDegree')}
        />
        <Filter
          subTitle="Ocena pracy w zespole scrum"
          listComponents={ratingsContents('teamProjectDegree')}
        />
        <Filter
          subTitle="Preferowane miejsce pracy"
          listComponents={workPlaceContents('expectedTypeWork')}
        />
        <Filter
          subTitle="Oczekiwany typ kontraktu "
          listComponents={workTypeContents('expectedContractType')}
        />
        <Filter
          subTitle="Oczekiwane wynagrodzenie miesięczne netto "
          listComponents={expectedSalaryContents('expectedSalary')}
        />
        <Filter
          subTitle="Zgoda na odbycie bezpłatnych praktyk/stażu na początek "
          listComponents={canTakeApprenticeshipContents(
            'canTakeApprenticeship',
          )}
          classes="filtersPanel__options-list--column"
        />
        <Filter
          subTitle="Ilość miesięcy doświadczenia komercyjnego kandydata w programowaniu "
          listComponents={commercialExpContents('monthsOfCommercialExp')}
        />

        <section className="filtersPanel__bottom">
          <button className="filtersPanel__cancel-btn" onClick={closeCallback}>
            Anuluj
          </button>
          <button
            className="filtersPanel__show-results-btn"
